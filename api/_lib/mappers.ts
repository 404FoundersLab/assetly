...
export function mapTenant(row: DbTenant) {
  return {
    ...
    name: row.name,
    slug: row.slug,
    plan: (row as any).plan ?? (row as any).subscription_plan_id ?? 'Enterprise',
    domain: (row as any).domain ?? undefined,
    infrastructureStrategy: (row as any).infrastructure_strategy ?? 'shared',
    adminEmail: (row as any).admin_email ?? undefined,
    adminName: (row as any).admin_name ?? undefined,
    subscriptionStatus: (row as any).subscription_status ?? (row as any).status ?? undefined,
    trialEndsAt: (row as any).trial_ends_at ?? undefined,
